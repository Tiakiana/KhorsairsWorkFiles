...
public class XWingMovement : MonoBehaviour
{
    ...
    public GameObject FlightManagers;
    ...
    public void TakeDamage(int dam) {
        Health -= dam;
        Debug.Log(gameObject.name + " is damaged for " + dam + "points");
        if (Health<1)
        {
            FlightManager.inst.RemovePlayer(gameObject.name);
            FlightManager.inst.Splosion.transform.position = transform.position;
            FlightManager.inst.Splosion.GetComponent<AnimationScript>().StartAnimBoom();
            Destroy(gameObject);
        }
    }
	...
	thing = new GameObject();
        FlightManagers = GameObject.FindGameObjectWithTag("FlightManager");
   ...
   /* public void Straight()
    ...
    void OnMouseDown()
    {
       FlightManagers.GetComponent<FlightManager>().StartSetDirection(gameObject);
    }
